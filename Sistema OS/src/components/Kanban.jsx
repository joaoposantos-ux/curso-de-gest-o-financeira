...
function Kanban({ osList, onDragEnd, usuario, onUpdateOs, osIdSelecionadaInicial }) {
  ...
  const [modalAviso, setModalAviso] = useState({ open: false, mensagem: '' });
  const [modalFilaDesenv, setModalFilaDesenv] = useState({ open: false, loading: false });
  const columns = ['Pendente', 'Em Andamento', 'Concluído'];
  const API_URL = import.meta.env.VITE_API_URL;
  // Troque o osList por um estado local para poder atualizar via socket
  const [osListLocal, setOsListLocal] = useState(osList);
  useEffect(() => {
    fetch(`${API_URL}/os`)
      .then(res => res.json())
      .then(data => setOsListLocal(Array.isArray(data) ? data : []));
  }, [API_URL]);
  // Sempre que o prop osList mudar (ex: ao carregar a página), sincronize:
  useEffect(() => {
    setOsListLocal(osList);
  }, [osList]);

  // Conecte ao socket e atualize a lista em tempo real
  useEffect(() => {
    const socket = io(`${API_URL}`); // ajuste para o endereço do seu backend
    socket.on('osAtualizada', (novaLista) => {
      setOsListLocal(novaLista);
    });
  ...
  const osFiltradas = filtroResponsavel
    ? osListLocal.filter(os => String(os.usuario_responsavel_id) === filtroResponsavel)
    : osListLocal;
  ...
  const handleDragEnd = (result) => {
    ...
    const osId = draggableId;
    const destinoStatus = columns[destination.droppableId] || destination.droppableId;

    // Encontre a OS que está sendo movida
    const os = osList.find(o => o.id.toString() === osId.toString());

    // Validação: se for para "Concluído", precisa ter solução proposta
    if (destinoStatus === "Concluído" && (!os.solucao_proposta || os.solucao_proposta.trim() === "")) {
      setModalAviso({
        open: true,
        mensagem: "Para concluir a OS, é necessário preencher o campo 'Solução'."
      });
      return;
    }
    ...
    <>
                        ...
                        <Draggable draggableId={os.id.toString()} index={idx} key={os.id}>
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={`relative bg-gray-50 border border-gray-100 rounded-lg p-4 shadow-sm transition-shadow duration-200 cursor-pointer ${snapshot.isDragging ? 'ring-2 ring-blue-300' : ''}`}
                            >
                              {/* ID destacado */}
                              <div className="absolute -top-3 -left-3 z-10">
                                <div className="w-9 h-9 flex items-center justify-center rounded-full bg-blue-600 text-white font-bold shadow-lg border-2 border-white text-sm">
                                  {os.id}
                                </div>
                              </div>
                              <h3 className="font-semibold text-gray-800 flex items-center justify-between">
                                ...
                                <span className="flex items-center gap-1">
                                  ...
                                  {/* Botão Visualizar */}
                                  <button
                                    className="p-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 flex items-center justify-center ml-1"
                                    onClick={e => {
                                      e.stopPropagation();
                                      setOsSelecionada(os);
                                    }}
                                  ...
                                  {/* Botão Deletar */}
                                  <button
                                    className="p-2 bg-red-500 text-white rounded-full hover:bg-red-600 flex items-center justify-center ml-1"
                                    onClick={e => {
                                      e.stopPropagation();
                                      setOsParaDeletar(os);
                                    }}
                              ...
                              {/* Adicione esta linha para mostrar o cliente */}
                              <div className="text-xs text-blue-700 font-semibold mb-1">
                                {os.empresa_solicitante || os.cliente_nome || '-'}
                              </div>
                              <div className="text-xs text-gray-500 mt-1">
                                <div>
                                  <b>{modulos.find(m => m.id === os.sistema_modulo_id)?.nome || '-'}
                                    <span className="mx-1">/</span>
                                    {funcionalidades.find(f => f.id === os.tela_funcionalidade_id)?.nome || '-'}
                                  </b>
                                </div>
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <div className="mt-1 text-sm text-blue-700 font-semibold">
                  Cliente: {osSelecionada.empresa_solicitante || osSelecionada.cliente_nome || '-'}
                </div>
                <div className="text-sm text-gray-700">
                  Telefone: {osSelecionada.telefone_solicitante || '-'}
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-500 mt-1">
                  ...
                  <span className={`ml-2 px-2 py-1 rounded text-white text-xs ${osSelecionada.status === 'Concluído' ? 'bg-green-500' : osSelecionada.status === 'Em Andamento' ? 'bg-blue-500' : 'bg-yellow-500'}`}>
            ...
            >
              <div className="space-y-4 mt-4">
                ...
                <div>
                  <label className="block font-semibold text-gray-700 mb-1">Solução:</label>
                  <textarea
            ...
            {/* Botão para mudar status para Fila Desenv */}
            <button
              className="mt-2 bg-purple-600 text-white px-4 py-1 rounded shadow hover:bg-purple-700 transition w-full"
              onClick={() => setModalFilaDesenv({ open: true, loading: false })}
            >
              Mover para Fila de Desenvovimento
            </button>
      ...
      {modalAviso.open && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-xl shadow-2xl p-8 max-w-xs w-full text-center border border-yellow-300">
            <div className="flex flex-col items-center mb-4">
              ...
              <h2 className="text-xl font-bold text-yellow-700 mb-2">Atenção</h2>
              <p className="text-gray-700 mb-4">{modalAviso.mensagem}</p>
            </div>
            <button
              className="px-6 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600 font-semibold"
              onClick={() => setModalAviso({ open: false, mensagem: '' })}
            >
              OK
            </button>
          </div>
      ...
      {/* Modal de confirmação para mover para Fila Desenv */}
      {modalFilaDesenv.open && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-xl shadow-2xl p-8 max-w-xs w-full text-center border border-purple-300">
            <div className="flex flex-col items-center mb-4">
              ...
              <h2 className="text-xl font-bold text-purple-700 mb-2">Atenção</h2>
              <p className="text-gray-700 mb-4">
                Tem certeza que deseja mover esta OS para <b>Fila de Desenvovimento</b>?<br />
                Essa ação pode impactar o fluxo do time.
              </p>
            </div>
            <div className="flex gap-3 justify-center">
              <button
                className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300"
                onClick={() => setModalFilaDesenv({ open: false, loading: false })}
                disabled={modalFilaDesenv.loading}
              >
                Cancelar
              </button>
              <button
                ...
                disabled={modalFilaDesenv.loading}
                onClick={async () => {
                  setModalFilaDesenv({ open: true, loading: true });
                  await fetch(`${API_URL}/os/${osSelecionada.id}/status`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ status: 'Fila Desenv' }),
                  });
                  setOsSelecionada(null); // Fecha o modal de visualização
                  if (onUpdateOs) onUpdateOs({ ...osSelecionada, status: 'Fila Desenv' });
                  setShowToast(true);
                  setTimeout(() => setShowToast(false), 2500);
                  setModalFilaDesenv({ open: false, loading: false });
                }}
              >
                Confirmar
              </button>
